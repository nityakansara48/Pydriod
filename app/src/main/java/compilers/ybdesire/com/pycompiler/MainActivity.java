...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      /*  //Admob
        ...
        //Buttons
        Button btn=findViewById(R.id.button_def);
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                editText.getText().insert(editText.getSelectionStart(), "if new_condition:");
            }
        });
        btn=findViewById(R.id.button_def);
        btn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                editText.getText().insert(editText.getSelectionStart(), "def fun_name():");
            }
        });
                    ...
                    public void run() {
                        ...
                        try  {
                            ...
                            try {
                                ...
                                {
                                    if(jdata.get("Errors").toString().contains("NameError")) {
                                        System.out.println(jdata.get("Errors").toString().length());
                                        int size = jdata.get("Errors").toString().split("\n").length;
                                        System.out.println(jdata.get("Errors").toString().split("\n")[size-1]);
                                        String s = jdata.get("Errors").toString().split("\n")[size-1];
                                        int i = 17;
                                        String ans = "";
                                        while(!Character.toString(s.charAt(i)).equals("'")){
                                            System.out.println(s.charAt(i));
                                            ans = ans + s.charAt(i);
                                            i++;
                                        }

                                        ans = editText.getText().toString().replace(ans, "print");

                                        System.out.println(ans);

                                        JsonObject postData1 = new JsonObject();
                                        postData1.addProperty("LanguageChoice", "5");
                                        postData1.addProperty("Program", ans);
                                        postData1.addProperty("Input", "enter_the_input_to_your_code_here");

                                        RequestBody body1 = RequestBody.create(mediaType, postData1.toString());
                                        ...
                                        JSONObject jdata1 = new JSONObject(responseBody1);
                                        setText(txtOutput,  jdata1.get("Result").toString()+"\nThere is a Name error in previous code which is solved automatically.");
                                        setText(editText, ans);

                                    }

                                    else if(jdata.get("Errors").toString().contains("ZeroDivisionError")) {

                                        String ans = editText.getText().toString().replace("/0", "/1");
                                        System.out.println(ans);
                                        JsonObject postData1 = new JsonObject();
                                        postData1.addProperty("LanguageChoice", "5");
                                        postData1.addProperty("Program", ans);
                                        ...
                                        JSONObject jdata1 = new JSONObject(responseBody1);
                                        setText(txtOutput, jdata1.get("Result").toString()+"\nThere is a ZeroDivisionError error (/0) in previous code which is replaced by /1.");
                                        setText(editText, ans);
                                    }

                                    else if(jdata.get("Errors").toString().contains("IndentationError: unexpected indent")) {

                                        int size = editText.getText().toString().split("\n").length;
                                        String ans = editText.getText().toString().split("\n")[size-1];
                                        ans = ans.replace(" ","");
                                        String f_ans = "";
                                        for (int j=0; j<size-1; j++)
                                            f_ans = f_ans + editText.getText().toString().split("\n")[j] + "\n";
                                        f_ans = f_ans + ans;
                                        System.out.println(f_ans);
                                        JsonObject postData1 = new JsonObject();
                                        postData1.addProperty("LanguageChoice", "5");
                                        postData1.addProperty("Program", f_ans);
                                        postData1.addProperty("Input", "enter_the_input_to_your_code_here");
                                        ...
                                        JSONObject jdata1 = new JSONObject(responseBody1);
                                        setText(txtOutput, jdata1.get("Result").toString()+"\nThere is an additional Indentation in previous code which is removed automatically.");
                                        setText(editText, f_ans);



                                    }

                                    else if(jdata.get("Errors").toString().contains("IndentationError: expected an indented block")) {
                                        ...
                                        JSONObject jdata1 = new JSONObject(responseBody1);
                                        setText(txtOutput, jdata1.get("Result").toString()+"\nThere is an Indentation required in previous code which is added automatically.");
                                        setText(editText, f_ans);
